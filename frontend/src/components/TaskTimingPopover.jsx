...
export default function TaskTimingPopover({ taskId, isDemo, sourceJob, compact = false }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`inline-flex items-center gap-1 font-medium transition-colors hover:opacity-80 max-w-full ${
          compact ? 'text-xs px-2 py-0.5 rounded-md' : 'text-sm'
        }`}
        style={{
          color: 'var(--text-2)',
          background: compact ? 'var(--bg-sub)' : 'transparent',
          border: compact ? '1px solid var(--border)' : 'none',
        }}
        aria-expanded={open}
        title={`Задача ${effectiveId}`}
      >
        {!compact && <span style={{ color: 'var(--muted)' }}>·</span>}
        <span
          className={compact ? 'text-[10px] uppercase tracking-wide' : ''}
          style={compact ? { color: 'var(--muted)' } : undefined}
        >
          задача
        </span>
        <span className="font-mono truncate">
          {compact ? effectiveId.slice(0, 8) : effectiveId}
        </span>
        <ChevronDown
          className={`flex-shrink-0 transition-transform ${compact ? 'w-3 h-3' : 'w-4 h-4'} ${open ? 'rotate-180' : ''}`}
