...
export function LiveDemoToggle({ enabled, onToggle }) {
  return (
    <button
      ...
      title={enabled ? 'Выключить live-поток' : 'Включить live-поток обращений'}
    >
...
export default function LiveDemoPanel({ enabled, feed }) {
  const { events, received, total, dismiss, error, source } = feed
        ...
        >
          ...
          >
            ...
            <span>
              ...
              {source === 'citizen' ? ' · обращения граждан' : ''}
            </span>
          </div>
          {error && (
            <div
              className="pointer-events-auto rounded-lg px-3 py-2 text-xs shadow-md border text-red-600"
