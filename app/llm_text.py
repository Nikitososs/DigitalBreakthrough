...
def _strip_markdown_paragraph(text: str) -> str:
    """Убирает markdown, сохраняя абзацы (пустые строки между блоками)."""
    t = text.strip()
    t = re.sub(r"^#+\s*.+$", "", t, flags=re.MULTILINE)
    t = re.sub(r"^>\s?", "", t, flags=re.MULTILINE)
    t = re.sub(r"\*\*(.+?)\*\*", r"\1", t)
...
def _truncate_paragraphs(text: str, max_chars: int) -> str:
    ...
    for block in blocks:
        ...
        if len(sentences) > 1:
            kept.append(" ".join(sentences[:-1]))
        elif sentences:
            kept.append(sentences[0].rstrip(".,!?…") + "…")
        break
    return "\n\n".join(kept)
...
def normalize_llm_summary(text: str, *, one_sentence: bool = True, max_chars: int = 800) -> str:
    ...
    raw = str(text).strip()
    strip_fn = _strip_markdown if one_sentence else _strip_markdown_paragraph
    if not _looks_like_meta(raw) and not raw.startswith("###") and "**" not in raw[:80]:
        core = strip_fn(raw)
    else:
        core = strip_fn(_extract_core(raw))
    ...
    if one_sentence:
        ...
        core = _truncate_paragraphs(core, max_chars)
