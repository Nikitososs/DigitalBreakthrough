...
# Высота frame SimpleDocTemplate минус заголовок секции (для KeepTogether + график)
_CATEGORY_CHART_MAX_HEIGHT = A4[1] - MARGIN_T - MARGIN_B - 32 * mm
CARD_RADIUS = 3.5 * mm
PAGE_HEADER_H = 16 * mm
LOGO_SIZE = 5.5 * mm
LOGO_RADIUS = 1.2 * mm
...
def _categories_chart(themes: list[ThematicGroupStat], width_mm: float) -> Image | None:
    if not themes:
        return None
    themes = sorted(
        [t for t in themes if t.count > 0],
        key=lambda t: t.count,
        reverse=True,
    )[:CATEGORIES_CHART_TOP_N]
    if not themes:
        return None
    _ensure_mpl()
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Нерешённые обращения", fontsize=9, labelpad=6)
    ...
    return _fig_to_image(fig, width_mm, max_height=_CATEGORY_CHART_MAX_HEIGHT)


def _severity_chart(stats: list[SeverityStat], width_mm: float) -> Image | None:
    filtered = [s for s in stats if s.severity > 0 and s.count > 0]
    if not filtered:
        return None
    _ensure_mpl()
...
def _problem_count_from_severity(stats: list[SeverityStat]) -> int:
    return sum(int(s.count) for s in stats if s.severity > 0)


def _district_resolved_metrics(report: DistrictReport) -> dict | None:
    """Решено / не решено по проблемным обращениям (если есть данные)."""
    problem = report.problem_count
    if problem is None:
        problem = _problem_count_from_severity(report.severity_stat)
    problem = int(problem or 0)
    if problem <= 0:
        return None

    resolved = report.resolved_count
    pct = report.resolved_pct
    if resolved is None and pct is not None:
        resolved = int(round(problem * float(pct) / 100.0))
    if resolved is None:
        return None

    resolved = max(0, min(problem, int(resolved)))
    if pct is None:
        pct = round(100.0 * resolved / problem, 1)
    open_count = problem - resolved
    return {
        "problem_count": problem,
        "resolved_count": resolved,
        "open_count": open_count,
        "resolved_pct": float(pct),
    }


def _aggregate_region_resolved(
    report: dict | None,
    ...
    if report and report.get("resolved_stats"):
        for row in report["resolved_stats"]:
            ...
            has_data = True
            total_problem += pc
            total_resolved += int(row.get("resolved_count") or 0)
    else:
        for district in districts:
            metrics = _district_resolved_metrics(district)
            if not metrics:
                continue
            has_data = True
            total_problem += metrics["problem_count"]
            total_resolved += metrics["resolved_count"]

    if not has_data or total_problem <= 0:
        return None

    total_resolved = max(0, min(total_problem, total_resolved))
    return {
        "problem_count": total_problem,
        "resolved_count": total_resolved,
        "open_count": total_problem - total_resolved,
        "resolved_pct": round(100.0 * total_resolved / total_problem, 1),
    }


def _resolved_pct_label(pct: float | None) -> str:
    if pct is None:
        return "—"
    return f"{pct:.1f}%".replace(".0%", "%")


def _resolved_progress_bar(pct: float, width: float) -> Table:
    pct_clamped = max(0.0, min(100.0, float(pct)))
    resolved_w = max(1.0, width * pct_clamped / 100.0)
    open_w = max(1.0, width - resolved_w)
    bar = Table([[""] * 2], colWidths=[resolved_w, open_w], rowHeights=[5 * mm])
    bar.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (0, 0), _hex("#16a34a")),
                ("BACKGROUND", (1, 0), (1, 0), _hex("#fecaca")),
...
def _resolved_status_block(report: DistrictReport, styles: dict, *, show_heading: bool = True) -> list:
    ...
    resolved_style = ParagraphStyle(
        ...
        textColor=_hex("#15803d"),
    )
    open_style = ParagraphStyle(
        "resolved_value_open",
        parent=styles["kpi_value"],
        textColor=_hex("#c2410c"),
    )
    pct_style = ParagraphStyle(
        "resolved_value_pct",
        parent=styles["kpi_value"],
        textColor=_hex("#1d4ed8"),
    )

    table = Table(
        [
            [
                Paragraph("Решено", styles["kpi_label"]),
                Paragraph("Не решено", styles["kpi_label"]),
                Paragraph("Доля решённых", styles["kpi_label"]),
            ],
            [
                Paragraph(str(metrics["resolved_count"]), resolved_style),
                Paragraph(str(metrics["open_count"]), open_style),
                Paragraph(_resolved_pct_label(metrics["resolved_pct"]), pct_style),
            ],
            [
                Paragraph(
                    f"из {metrics['problem_count']} проблемных обращений",
                    styles["kpi_label"],
                ),
                Paragraph("открытые проблемы", styles["kpi_label"]),
                Paragraph("по полю «Итог» (AD)", styles["kpi_label"]),
            ],
        ],
    ...
    table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), _hex("#f0fdf4")),
                ("BACKGROUND", (0, 1), (-1, 1), colors.white),
                ("BACKGROUND", (0, 2), (-1, 2), _hex("#f8fafc")),
                ("INNERGRID", (0, 0), (-1, -1), 0.25, _hex(C_BORDER)),
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
    ...
    legend = Table(
        [
            [
                Paragraph('<font color="#15803d">■</font> Решено', styles["kpi_label"]),
                Paragraph('<font color="#c2410c">■</font> Не решено', styles["kpi_label"]),
            ]
        ],
    ...
    flow: list = []
    if show_heading:
        flow.extend([_section_heading("Решённость проблемных обращений", styles), Spacer(1, 3 * mm)])
    flow.append(_rounded_card(table, bg="#ffffff", border=C_BORDER))
    flow.append(Spacer(1, 2 * mm))
    flow.append(_resolved_progress_bar(metrics["resolved_pct"], CONTENT_W - 4 * mm))
    flow.append(Spacer(1, 2 * mm))
    flow.append(legend)
...
def _region_resolved_block(
    report: dict | None,
    districts: list[DistrictReport],
    styles: dict,
) -> list:
    metrics = _aggregate_region_resolved(report, districts)
    if not metrics:
        return []

    body = Paragraph(
        escape(
            f"По области среди проблемных обращений решено "
            f"{metrics['resolved_count']:,}".replace(",", " ")
            + f" из {metrics['problem_count']:,}".replace(",", " ")
            + f" ({_resolved_pct_label(metrics['resolved_pct'])}). "
            f"Без решения остаётся {metrics['open_count']:,}".replace(",", " ")
            + " обращений."
        ),
        styles["body"],
    )
    inner = Table([[body]], colWidths=[CONTENT_W - 16 * mm])
    inner.setStyle(
        TableStyle(
            [
                ("LEFTPADDING", (0, 0), (-1, -1), 0),
    ...
    card = Table([[inner], [_resolved_progress_bar(metrics["resolved_pct"], CONTENT_W - 20 * mm)]], colWidths=[CONTENT_W])
    card.setStyle(
        TableStyle(
            [
                ("LEFTPADDING", (0, 0), (-1, -1), 10),
    ...
    return [
        _section_heading("Решённость проблемных обращений (область)", styles),
        Spacer(1, 3 * mm),
        _rounded_card(card, bg="#f0fdf4", border="#86efac"),
        Spacer(1, 8 * mm),
    ]


def _enrich_resolved_fields(
    row: dict,
    districts: list[DistrictReport],
    report: dict | None,
) -> dict:
    muni = str(row.get("муниципалитет", "")).strip()
    out = dict(row)
    for district in districts:
        if district.district_name == muni:
            metrics = _district_resolved_metrics(district)
            if metrics:
                out.update(metrics)
            break
    if out.get("resolved_pct") is None and report:
        for stats_row in report.get("resolved_stats") or []:
            if str(stats_row.get("муниципалитет", "")).strip() == muni:
                pc = int(stats_row.get("problem_count") or 0)
                rc = int(stats_row.get("resolved_count") or 0)
                if pc > 0:
                    out["problem_count"] = pc
                    out["resolved_count"] = rc
                    out["open_count"] = pc - rc
                    out["resolved_pct"] = float(stats_row.get("resolved_pct") or round(100.0 * rc / pc, 1))
                break
    return out


def _period_meta(stats: dict | None, districts: list[DistrictReport]) -> str:
    start = end = None
    if stats:
        start = stats.get("start_date")
        end = stats.get("end_date")
    if not start and districts:
        start = districts[0].start_date
        end = districts[0].end_date
    period = ""
    if start or end:
        s = start.strftime("%d.%m.%Y") if hasattr(start, "strftime") else str(start or "—")
        e = end.strftime("%d.%m.%Y") if hasattr(end, "strftime") else str(end or "—")
        period = f"Период: {s} — {e} · "
    return period


def _criticality_badge(score: int, severity_mean: float = 0.0) -> str:
...
def _region_kpi_items(stats: dict | None, districts: list[DistrictReport], report: dict | None) -> list[tuple[str, str]]:
    stats = stats or {}
    total = stats.get("rows_processed") or stats.get("total_incidents")
    if total is None:
        total = sum(int(d.total_incidents) for d in districts)
    problems = stats.get("problem_count")
    if problems is None and report:
        all_rows = report.get("all") or []
        if all_rows and any(r.get("problem_count") is not None for r in all_rows):
            problems = sum(int(r.get("problem_count") or 0) for r in all_rows)
    if problems is None:
        problems = sum(_problem_count_from_severity(d.severity_stat) for d in districts)
    muni_count = len(districts)
    if report and report.get("all"):
        muni_count = len(report["all"])
    top10_n = len(report.get("top10") or []) if report else min(10, muni_count)
    return [
        ("Всего обращений", _fmt_int(total or 0)),
        ("Проблемных", _fmt_int(problems or 0)),
        ("Муниципалитетов", str(muni_count)),
        ("В Top-10", str(top10_n)),
    ]


def _region_kpi_block(
    stats: dict | None,
    districts: list[DistrictReport],
    styles: dict,
    *,
    ...
    items = _region_kpi_items(stats, districts, report)
    cells = [
    ...
    table = Table(cells, colWidths=[col_w] * len(items))
    table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, -1), _hex("#fef2f2")),
                ("INNERGRID", (0, 0), (-1, -1), 0.25, _hex(C_BORDER)),
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
                ("LEFTPADDING", (0, 0), (-1, -1), 6),
...
def _aggregate_region_topics(topics: list[dict], limit: int = 12) -> list[tuple[str, int]]:
    weights: Counter[str] = Counter()
    for row in topics:
        theme = str(row.get("тема", "")).strip()
        if theme:
            weights[theme] += int(row.get("count") or 0)
    return weights.most_common(limit)


def _aggregate_region_severity(breakdown: list[dict]) -> list[tuple[int, str, int]]:
    totals: Counter[tuple[int, str]] = Counter()
    for row in breakdown:
        sev = int(row.get("severity") or 0)
        if sev <= 0:
            continue
        label = str(row.get("label") or f"Класс {sev}")
        totals[(sev, label)] += int(row.get("count") or 0)
    return sorted(((sev, label, cnt) for (sev, label), cnt in totals.items()), key=lambda x: x[0])


def _top10_rows(report: dict | None, districts: list[DistrictReport]) -> list[dict]:
    if report and report.get("top10"):
        rows = [_enrich_resolved_fields(row, districts, report) for row in list(report["top10"])[:10]]
        return rows
    ordered = sorted(districts, key=lambda d: d.score, reverse=True)[:10]
    rows = [
        {
            "rank": i + 1,
            "муниципалитет": d.district_name,
            "score": d.score,
            "total_incidents": d.total_incidents,
            "problem_count": _problem_count_from_severity(d.severity_stat),
            "critical_count": sum(s.count for s in d.severity_stat if s.severity >= 4),
            "топ_тема": d.top_category,
        }
        for i, d in enumerate(ordered)
    ]
    return [_enrich_resolved_fields(row, districts, report) for row in rows]


def _top3_rows(report: dict | None, districts: list[DistrictReport]) -> list[dict]:
    if report and report.get("top3"):
        return [_enrich_resolved_fields(row, districts, report) for row in list(report["top3"])[:3]]
    ordered = sorted(districts, key=lambda d: d.score, reverse=True)[:3]
    rows = [
        {
            "муниципалитет": d.district_name,
            "score": d.score,
            ...
            "severity_mean": 0.0,
            "топ_тема": d.top_category,
            "summary_text": d.analytical_summary,
            "summary_paragraph": d.analytical_summary,
        }
        for d in ordered
    ]
    return [_enrich_resolved_fields(row, districts, report) for row in rows]
...
def _top10_ranking_table(rows: list[dict], styles: dict) -> Table:
    col_num = 9 * mm
    col_score = 14 * mm
    col_total = 18 * mm
    col_prob = 18 * mm
    ...
    col_crit = 14 * mm
    col_name = 40 * mm
    col_theme = CONTENT_W - col_num - col_score - col_total - col_prob - col_res - col_crit - col_name

    header = [
        "№",
        _para("Муниципалитет", styles["table_header"]),
        _para("Индекс", styles["table_header_center"]),
        _para("Всего", styles["table_header_center"]),
        _para("Проблемн.", styles["table_header_center"]),
        _para("Реш.", styles["table_header_center"]),
        _para("Крит.", styles["table_header_center"]),
        _para("Главная тема", styles["table_header"]),
    ]
    data_rows = [header]
    for i, row in enumerate(rows, 1):
        score = int(row.get("score", 0))
        _, fg = _score_palette(score)
        ...
        theme = str(row.get("топ_тема", row.get("ключевые_темы", ""))).split(";")[0].strip()
        resolved_count = row.get("resolved_count")
        problem_count = row.get("problem_count")
        if resolved_count is not None and problem_count:
            resolved_cell = f"{resolved_count}/{problem_count}"
        else:
            resolved_cell = _resolved_pct_label(row.get("resolved_pct")) if row.get("resolved_pct") is not None else "—"
        data_rows.append(
            [
                str(row.get("rank", i)),
                _para(muni, styles["table_cell"]),
                Paragraph(f"<b>{score}</b>", score_style),
                str(row.get("total_incidents", "")),
                str(row.get("problem_count", "")),
                resolved_cell,
                str(row.get("critical_count", "")),
                _para(theme[:80], styles["table_cell"]),
            ]
        )

    table = Table(
        data_rows,
        colWidths=[col_num, col_name, col_score, col_total, col_prob, col_res, col_crit, col_theme],
        repeatRows=1,
    )
    ...
    for i, row in enumerate(rows, 1):
        bg, fg = _score_palette(int(row.get("score", 0)))
        style.append(("BACKGROUND", (2, i), (2, i), _hex(bg)))
        style.append(("TEXTCOLOR", (2, i), (2, i), _hex(fg)))
        style.append(("FONT", (2, i), (2, i), FONT_BOLD, 8.5))
    table.setStyle(TableStyle(style))
    return table


def _top10_score_chart(rows: list[dict], width_mm: float) -> Image | None:
    if not rows:
        return None
    _ensure_mpl()
    labels = [_chart_label(str(r.get("муниципалитет", "")), max_len=36) for r in rows]
    scores = [float(r.get("score", 0)) for r in rows]
    bar_colors = [_score_palette(int(s))[1] for s in scores]

    bar_h = 0.55
    fig_h = max(3.0, len(rows) * bar_h + 1.0)
    fig, ax = plt.subplots(figsize=(7.4, fig_h))
    y_pos = list(range(len(labels)))
    bars = ax.barh(y_pos, scores, color=bar_colors, height=0.72, edgecolor="white", linewidth=0.6)
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels, fontsize=8.5)
    ax.invert_yaxis()
    ax.set_xlabel("Индекс проблемности", fontsize=9, labelpad=6)
    ax.spines[["top", "right"]].set_visible(False)
    ax.grid(axis="x", alpha=0.9)
    ax.margins(x=0.08)
    for bar, val in zip(bars, scores):
        ax.text(
            bar.get_width() + max(scores or [1]) * 0.015,
            bar.get_y() + bar.get_height() / 2,
            str(int(val)),
            va="center",
            fontsize=8,
    ...
    fig.subplots_adjust(left=0.34, right=0.96, top=0.96, bottom=0.10)
    return _fig_to_image(fig, width_mm, max_height=95 * mm)


def _region_topics_chart(topics: list[tuple[str, int]], width_mm: float) -> Image | None:
    if not topics:
        return None
    _ensure_mpl()
    labels = [_chart_label(name) for name, _ in topics[:10]]
    counts = [cnt for _, cnt in topics[:10]]
    bar_colors = [CATEGORY_BAR_COLORS[min(i, len(CATEGORY_BAR_COLORS) - 1)] for i in range(len(labels))]

    fig_h = max(3.0, len(labels) * 0.55 + 1.0)
    fig, ax = plt.subplots(figsize=(7.4, fig_h))
    y_pos = list(range(len(labels)))
    bars = ax.barh(y_pos, counts, color=bar_colors, height=0.72, edgecolor="white", linewidth=0.6)
    ax.set_yticks(y_pos)
    ...
    fig.subplots_adjust(left=0.38, right=0.96, top=0.96, bottom=0.10)
    return _fig_to_image(fig, width_mm, max_height=85 * mm)


def _region_severity_chart(breakdown: list[dict], width_mm: float) -> Image | None:
    agg = _aggregate_region_severity(breakdown)
    if not agg:
        return None
    stats = [SeverityStat(severity=sev, label=label, count=cnt, percentage=0.0) for sev, label, cnt in agg]
    return _severity_chart(stats, width_mm)


def _critical_district_card(row: dict, styles: dict) -> RoundedCard:
    muni = str(row.get("муниципалитет", ""))
    score = int(row.get("score", 0))
    badge = _criticality_badge(score, float(row.get("severity_mean") or 0))
    prob = row.get("problem_count", "")
    total = row.get("total_incidents", "")
    theme = str(row.get("топ_тема", row.get("ключевые_темы", ""))).split(";")[0].strip()
    resolved_line = ""
    if row.get("resolved_count") is not None and row.get("problem_count"):
        resolved_line = (
            f" · решено <b>{row['resolved_count']}</b> из <b>{row['problem_count']}</b>"
            f" ({_resolved_pct_label(row.get('resolved_pct'))})"
        )
    summary = str(
        row.get("summary_paragraph") or row.get("summary_text") or row.get("аналитическая_справка") or ""
    ).strip()
    if summary:
        summary = escape(_clean_text(summary))[:320]
        if len(summary) >= 320:
            summary += "…"
    ...
    lines = [
        [Paragraph(f'<font color="{C_BRAND}">{escape(badge)}</font> · {escape(muni)}', badge_style)],
        [
            Paragraph(
                f"Индекс <b>{score}</b> · проблемных <b>{prob}</b> из <b>{total}</b>"
                + resolved_line
                + (f" · тема «{escape(theme)}»" if theme else ""),
                styles["body"],
            )
        ],
    ]
    if summary:
        lines.append([Paragraph(summary, styles["summary"])])

    inner = Table(lines, colWidths=[CONTENT_W - 24 * mm])
    inner.setStyle(
        TableStyle(
            [
                ("LEFTPADDING", (0, 0), (-1, -1), 0),
    ...
    return _rounded_card(card, bg="#fef2f2", border="#fca5a5")


def _critical_districts_block(rows: list[dict], styles: dict) -> list:
    if not rows:
        return []
    flow: list = [_section_heading("Top-3 критичных муниципалитетов", styles), Spacer(1, 3 * mm)]
    for row in rows[:3]:
        flow.append(_critical_district_card(row, styles))
...
def _kpi_table(report: DistrictReport, styles: dict) -> Table:
    ...
    problem_count = _problem_count_from_severity(report.severity_stat)
    cells = [
        [
            Paragraph("Всего обращений", styles["kpi_label"]),
            Paragraph("Проблемных", styles["kpi_label"]),
        ...
        [
            ...
            Paragraph(str(problem_count), styles["kpi_value"]),
...
def _shares_table(
    ...
    filtered = [
        t
        for t in themes
        if t.resolved_pct is None or t.resolved_pct < 100.0
    ]
    if not filtered or problem_total <= 0:
        return None
    has_resolved = any(t.resolved_pct is not None for t in filtered)
    rows = [
        [
    ...
    if has_resolved:
        rows[0].append(_para("Доля решённых", styles["table_header_center"]))
    for t in filtered:
        appeals = t.total_count if t.total_count is not None else t.count
        share = t.percentage
        if t.total_count is None and problem_total:
            share = round(appeals / problem_total * 100, 1)
        row = [
            _para(t.group_name, styles["table_cell"]),
            _para(str(appeals), styles["table_cell_center"]),
            _para(f"{share:.1f}%", styles["table_cell_center"]),
        ]
        if has_resolved:
            resolved_text = f"{t.resolved_pct:.1f}%" if t.resolved_pct is not None else "—"
            row.append(_para(resolved_text, styles["table_cell_center"]))
        rows.append(row)
    if has_resolved:
        col_widths = [CONTENT_W - 86 * mm, 24 * mm, 24 * mm, 38 * mm]
    else:
        col_widths = [CONTENT_W - 58 * mm, 28 * mm, 30 * mm]
...
def _severity_shares_table(stats: list[SeverityStat], total: int, styles: dict) -> Table | None:
    filtered = [s for s in stats if s.severity > 0 and s.count > 0]
    if not filtered:
        return None
    denom = total if total > 0 else sum(s.count for s in filtered)
    if denom <= 0:
        return None
    rows = [
        [
            _para("Класс тяжести", styles["table_header"]),
            _para("Обращений", styles["table_header_center"]),
            _para("Доля", styles["table_header_center"]),
        ]
    ]
    for s in filtered:
        color = SEVERITY_BAR_COLORS.get(s.severity, "#94a3b8")
        label_cell = Paragraph(
            f'<font color="{color}"><b>{escape(s.label)}</b></font>',
            styles["table_cell"],
        )
        rows.append(
            [
                label_cell,
                _para(str(s.count), styles["table_cell_center"]),
                _para(f"{s.percentage:.1f}%", styles["table_cell_center"]),
            ]
        )
...
def _district_story(
    ...
    *,
    ...
    examples_limit = 3 if compact else 5
    score_bg, score_fg = _score_palette(int(report.score))

    if section_title:
        period = ""
        if report.start_date or report.end_date:
            s = report.start_date.strftime("%d.%m.%Y") if report.start_date else "—"
            e = report.end_date.strftime("%d.%m.%Y") if report.end_date else "—"
            period = f"Период: {s} — {e} · "
        meta = (
            f"{period}Индекс {report.score} из 100 · обращений {report.total_incidents} · "
    ...
    story.extend(_resolved_status_block(report, styles))
    ...
    if cat_chart:
        story.append(
            KeepTogether(
                [
                    _section_heading("Обращения по категориям (нерешённые)", styles),
                    Spacer(1, 3 * mm),
                    cat_chart,
                ]
            )
    ...
    if sev_chart:
        story.append(
            KeepTogether(
                [
                    _section_heading("Распределение по тяжести (нерешённые)", styles),
                    Spacer(1, 3 * mm),
                    sev_chart,
                ]
            )
    ...
    unresolved_severity_total = sum(s.count for s in report.severity_stat if s.severity > 0)
    sev_table = _severity_shares_table(report.severity_stat, unresolved_severity_total, styles)
    if sev_table and not compact:
        story.append(
            KeepTogether(
                [
                    _section_heading("Доли по классам тяжести (нерешённые)", styles),
                    Spacer(1, 3 * mm),
                    sev_table,
                ]
            )
    ...
    shares = _shares_table(
        themes,
        report.problem_count or report.total_incidents,
        styles,
    )
    if shares:
        story.append(
...
def build_region_pdf(
    ...
    *,
    ...
    period = _period_meta(stats, ordered)

    story: list = _hero_header(
        "Все муниципалитеты",
        "Сводный отчёт по Омской области",
        f"{period}Сформирован: {datetime.now().strftime('%d.%m.%Y %H:%M')} · "
        f"Муниципалитетов: {len(ordered)}",
        styles,
    )
    story.append(_region_kpi_block(stats, ordered, styles, report=report))
    story.extend(_region_resolved_block(report, ordered, styles))
    story.append(Spacer(1, 8 * mm))
    ...
    top3 = _top3_rows(report, ordered)
    story.extend(_critical_districts_block(top3, styles))

    top10 = _top10_rows(report, ordered)
    if top10:
        story.append(_section_heading("Top-10 муниципалитетов", styles))
        story.append(Spacer(1, 3 * mm))
        story.append(_top10_ranking_table(top10, styles))
        story.append(Spacer(1, 6 * mm))
        top10_chart = _top10_score_chart(top10, CONTENT_W)
        if top10_chart:
            story.append(top10_chart)
            story.append(Spacer(1, 8 * mm))

    topics = _aggregate_region_topics((report or {}).get("topics") or [])
    if topics:
        topics_chart = _region_topics_chart(topics, CONTENT_W)
        if topics_chart:
            story.append(
                KeepTogether(
                    [
                        _section_heading("Топ тем обращений по области", styles),
                        Spacer(1, 3 * mm),
                        topics_chart,
                    ]
                )
    ...
    breakdown = (report or {}).get("severity_breakdown") or []
    sev_chart = _region_severity_chart(breakdown, CONTENT_W)
    if sev_chart:
        story.append(
            KeepTogether(
                [
                    _section_heading("Распределение по тяжести (область)", styles),
                    Spacer(1, 3 * mm),
                    sev_chart,
                ]
            )
    ...
    story.append(_section_heading("Все муниципалитеты — сводная таблица", styles))
    story.append(Spacer(1, 3 * mm))
